...
# processes have tid matching pid, with -T threads have their parent PID shown.
# place etime last because all other fields are constant in time (except cpuid because of scheduler) 
# ps's native sorting does not seem compatible with listing threads: revert to sort on tid
psfull="ps --no-header -eTo ppid,pid,tid,cpuid,cmd,lstart,etimes|sort -nk3"

# Recursively search for child processes and threads with awk
psold_full="$(eval $psfull | awk -vpp=$root_pid -vp=$meas_pid 'function r(s,e){if(s!=e) {print ps_string[s];s=child_id[s];while(s){sub(",","",s);t=s;sub(",.*","",t);sub("[0-9]+","",s);r(t,e)}}}{child_id[$1]=child_id[$1]","$3;ps_string[$3]=$0}END{r(pp,p)}')"
...
do
    sleep $sampl
    psnew_full="$(eval $psfull | awk -vpp=$root_pid -vp=$meas_pid 'function r(s,e){if(s!=e) {print ps_string[s];s=child_id[s];while(s){sub(",","",s);t=s;sub(",.*","",t);sub("[0-9]+","",s);r(t,e)}}}{child_id[$1]=child_id[$1]","$3;ps_string[$3]=$0}END{r(pp,p)}')"
    ...
    awk 'BEGIN{i=1; j=1} FILENAME==ARGV[1]&&/^[0-9,]+[cd][0-9,]+/ {split($0, a, "[cd]"); n=split(a[1], b, ","); if(n==1) {b[2]=b[1]}; for(k=b[1];k<=b[2];k++) {line[i++]=k}; next} FILENAME==ARGV[2]{psfull[j++]=$0; next} END{for(k=1;k<i;k++) {print psfull[line[k]]}}' <(echo "$difflines") <(echo "$psold_full") >> $journal

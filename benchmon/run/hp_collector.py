...
class HighPerformanceCollector:
    ...
    def start(self):
        ...
        for name in self.metric_types:
            ...
            p_thread = threading.Thread(target=producers[name], args=(self.queues[name],), daemon=True)
            ...
            c_thread = threading.Thread(
                ...
                args=(self.queues[name], self.clients[name], name),
                daemon=True,
            )
        ...
        if self.clients:
            self.reporter_thread = threading.Thread(
                target=self._report_queue_sizes,
                args=(10,),
                daemon=True,
            )
            self.reporter_thread.start()
        else:
            self.logger.warning("HP Collector did not initialize any InfluxDB clients.")

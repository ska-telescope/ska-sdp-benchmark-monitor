...
class HighPerformanceCollector:
        ...
        while not self.stop_event.wait(self.interval):
            try:
                ...
                for line in content:
                    ...
                    rx_bytes = parts[1]
                    tx_bytes = parts[9]
                    lp = f"network_stats,hostname={self.hostname},interface={iface} rx_bytes={rx_bytes}i,tx_bytes={tx_bytes}i {timestamp}"
                    points.append(lp)
        ...
        while not self.stop_event.wait(self.interval):
            try:
                ...
                for line in content:
                    ...
                    if not (device.startswith('sd') or device.startswith('nvme') or device.startswith('vd') or device.startswith('xvd')):
                        continue
                    
                    sectors_read = parts[5]
                    sectors_written = parts[9]
                    lp = f"disk_stats,hostname={self.hostname},device={device} sectors_read={sectors_read}i,sectors_written={sectors_written}i {timestamp}"
                    points.append(lp)
    ...
    def _ib_producer(self, q: queue.Queue):
        ...
        ib_ports = [p.split('/')[4] for p in glob.glob("/sys/class/infiniband/*/ports/*/counters/port_xmit_data")]
            ...
            try:
                ...
                for port in ib_ports:
                    try:
                        ...
                        lp = f"infiniband,hostname={self.hostname},device={port} port_rcv_data={rcv_data}i,port_xmit_data={xmit_data}i {timestamp}"
                        points.append(lp)
